    ...
    try {
      ...
      if (results.rowCount > 0) {
        ...
        if (visitor.rowCount > 0) {
          const visitNotification = await db.query('INSERT INTO notifications (user_id, from_id, notification) VALUES ($1, $2, $3) RETURNING *', [profile_id, user_id, `${visitor.rows[0].first_name.charAt(0).toUpperCase() + visitor.rows[0].first_name.slice(1)} visited your profile!`]);
...
//const isItAMatch = db.query("SELECT * likes WHERE user_id = $2 AND liked_user = $1", [user_id, profile_id]);
  exports.likeUser = async (req, res) => {
    ...
    const notification = await db.query('INSERT INTO notifications (user_id, from_id, notification) VALUES ($1, $2, $3) RETURNING *', [profile_id, user_id, `${first_name.charAt(0).toUpperCase() + first_name.slice(1)} liked your profile!`]);
    const isMatch = await db.query('SELECT * FROM likes WHERE user_id = $1 and liked_user = $2', [profile_id, user_id])
    console.log('isMatch: ');
    if (isMatch.rowCount > 0) {
      //console.log('It\'s a match!');
      const matched = await db.query('INSERT INTO conversations (user_one_id, user_two_id) VALUES ($1, $2) RETURNING *', [user_id, profile_id])
      const matchNotification = await db.query('INSERT INTO notifications (user_id, from_id, notification) VALUES ($1, $2, $3) RETURNING *', [profile_id, user_id, `You matched with ${first_name.charAt(0).toUpperCase() + first_name.slice(1)}!`]);
      const matchedUser = await db.query('SELECT * FROM users WHERE user_id = $1', [profile_id])
      console.log(matchedUser);
      if (matchedUser.rowCount > 0) {
        const matchNotification = await db.query('INSERT INTO notifications (user_id, from_id, notification) VALUES ($1, $2, $3) RETURNING *', [user_id, profile_id, `You matched with ${matchedUser.rows[0].first_name.charAt(0).toUpperCase() + matchedUser.rows[0].first_name.slice(1)}!`]);
      }//console.log(matched);
    } 
      return res.send(results)
      //return res.status(200)
    ...
    exports.getLike = async (req, res) => {
      ...
      exports.unlikeUser = async (req, res) => {
        ...
        //console.log(results)
        // To-do: insert notification.
        const notification = await db.query('INSERT INTO notifications (user_id, from_id, notification) VALUES ($1, $2, $3) RETURNING *', [profile_id, user_id, `${first_name.charAt(0).toUpperCase() + first_name.slice(1)} unliked your profile! D:`]);
