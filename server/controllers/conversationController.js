...
exports.insertMessageIntoConversation = async (message) => {
  ...
  console.log(message);
  try {
    const { message_text, sender_id, timestamp, conversation, partner } =
      message;

    const results = await db.query(
      `INSERT INTO messages(sender_id, conversation_id, message_text, timestamp) VALUES ($1, $2, $3, $4)`,
      [sender_id, conversation, message_text, timestamp]
    );
    const sender = await db.query("SELECT * FROM users WHERE user_id = $1", [
      sender_id,
    ]);
    //console.log(sender);
    if (sender.rowCount > 0) {
      const matchNotification = await db.query(
        ...
        [
          ...
          sender_id,
          `${
            sender.rows[0].first_name.charAt(0).toUpperCase() +
            sender.rows[0].first_name.slice(1)
          } sent you a message!`,
        ]
      );
    }
...
exports.getConversationsArray = async (req, res) => {
  ...
  try {
    ...
    const results = await db.query(
      `SELECT * FROM conversations WHERE user_one_id = $1 OR user_two_id = $1`,
      [user_id]
    );
    //console.log(results)
    return res.send(results);
