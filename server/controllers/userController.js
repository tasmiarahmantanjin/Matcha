...
exports.update = async (req, res) => {
  ...
  try {
    ...
    if (interest_arr) {
      let difference = interest_arr.filter(
        (item) => !user.interest.includes(item)
      );
      ...
      if (difference.length > 0) {
        for (let i in difference) {
          ...
          const hashtags = await db.query(
            `SELECT * FROM hashtags WHERE interest = $1`,
            [difference[i]]
          );
          if (hashtags.rowCount === 0) {
            console.log(`Inserting into table: ${difference[i]}`);
            db.query(`INSERT INTO hashtags(interest, number) VALUES ($1, $2)`, [
              difference[i],
              1,
            ]);
          }
      ...
      let deleted = user.interest.filter(
        (item) => !interest_arr.includes(item)
      );
      ...
      if (avatar !== null) {
        ...
        db.query("UPDATE users SET avatar = $1 WHERE email = $2", [
          avatar,
          email,
        ]);
