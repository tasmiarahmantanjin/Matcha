  ...
  try {
    ...
    if (interest_arr) {
      ...
      if (difference.length > 0) {
        for (let i in difference) {
          ...
          const hashtags = await db.query(`SELECT * FROM hashtags WHERE interest = $1`, [difference[i]]);
          if (hashtags.rowCount === 0){
          ...
          if (hashtags.rowCount > 0) {
            if (!user.interest.includes(difference[i])){
              db.query("UPDATE hashtags SET number = $1 WHERE interest = $2", [hashtags.rows[0].number + 1, difference[i]]);
      ...
      if (deleted.length > 0) {
        for (let i in deleted) {
          ...
          const hashtags = await db.query(`SELECT  * FROM hashtags WHERE interest = $1`, [deleted[i]]);
          db.query("UPDATE hashtags SET number = $1 WHERE interest = $2", [hashtags.rows[0].number - 1, deleted[i]]);
    ...
    if (avatar !== null) {
      ...
      const newImage = await db.query("INSERT INTO gallery (owner_id, path) VALUES ($1, $2) RETURNING path", [user_id, avatar]);
		...
		//return res.send(updatedUser) // Use this to send updatedUser data
  }
